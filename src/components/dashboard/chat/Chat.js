...
const Chat = React.memo(() => {
  ...
  const fetchConversationsAndMessages = useCallback(async () => {
    try {
      const response = await fetch(`${backendUrl}/get_user_conversations`, {
        method: "GET",
        headers: {
  ...
  }, [idToken, setConversationId, setConversationList, setMessages]);
  ...
  const fetchMessages = useCallback(
    async (conversationId) => {
      try {
        ...
        if (!messageResponse.ok) throw new Error("Failed to load messages");
