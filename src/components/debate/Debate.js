...
import ChatBar from "../dashboards/chat/chat_container/ChatBar";

import MessagesContainer from "../dashboards/chat/chat_container/MessagesContainer";
...
const Debate = ({ id, chatName, topic }) => {
  ...
  const startDebate = useCallback(
    async (turn = 0) => {
      try {
        const response = await fetch(`${backendUrl}/debate/start_debate`, {
          method: "POST",
          headers: {
          ...
          {messages.map((message, index) => {
            if (message) {
              if (message.message_from === "agent1") {
                return (
                  <DebateMessage key={index} message={message} agent="agent1" />
                );
              } else if (message.message_from === "agent2") {
                return (
                  <DebateMessage key={index} message={message} agent="agent2" />
                );
              }
            }
